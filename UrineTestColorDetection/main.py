...
img = cv2.imread("D:\\my_volume_input\\stick1.jpg")
...
blurred = cv2.GaussianBlur(resized, (51, 51), 0)
viewImage(blurred, "blurred")

output = resized.copy()
cv2.rectangle(output, (0, 136), (2560, 0), (0, 0, 0), 3)
viewImage(output, "rect")

'''
'''
# Opening the image from files
imaging = cv2.imread("D:\\my_volume_input\\stick1.jpg")
...
def color_detection_and_cropping_zone(img):
    ...
    borders_points = np.array([[points[0, 0], points[2, 0] + w * 3], [points[0, 1], points[3, 1] - r * 3],  # top-rect
                               [points[1, 0], points[2, 0] + w * 3], [points[1, 1], points[3, 1] - r * 3],
                               # bottom-rect
    ...
    for ii in range(0, 8, 2):
        ...
        border_colors[int(ii / 2)] = np.array(
            [np.median(crop[:, :, 0]), np.median(crop[:, :, 1]), np.median(crop[:, :, 2])]).astype(int)
    purpose_zone = image[int(points[0, 1] + q * 2):int(points[1, 0] - w * 2.0),
                   int(points[2, 1] + e * 1.5):int(points[3, 0] - r * 2.0)]
...
REFERENCE_IMAGE = 'D:\\normal.png'
ORIGIN_IMAGE = 'D:\\checker.png'

COLOUR_CHECKER_IMAGE_PATHS = ORIGIN_IMAGE
COLOUR_CHECKER_REFERENCE_IMAGE_PATHS = REFERENCE_IMAGE
COLOUR_CHECKER_IMAGES = [
    colour.cctf_decoding(colour.io.read_image(COLOUR_CHECKER_IMAGE_PATHS))
]
COLOUR_CHECKER_REFERENCE_IMAGES = [
    colour.cctf_decoding(colour.io.read_image(COLOUR_CHECKER_REFERENCE_IMAGE_PATHS))
]

SWATCHES = []
for image in COLOUR_CHECKER_IMAGES:
    for swatches, colour_checker, masks, *rest in detect_colour_checkers_segmentation(image, additional_data=True):
        swatches = np.asarray(swatches)
        swatches = swatches[1]
...
REFERENCE_SWATCHES = np.asarray(REFERENCE_SWATCHES)
REFERENCE_SWATCHES = np.squeeze(REFERENCE_SWATCHES)
print(REFERENCE_SWATCHES)

D65 = colour.CCS_ILLUMINANTS['CIE 1931 2 Degree Standard Observer']['D65']
REFERENCE_COLOUR_CHECKER = colour.CCS_COLOURCHECKERS['ColorChecker 2005']

matrix_RGB_to_XYZ = np.array([[0.4124564, 0.3575761, 0.1804375],
                              [0.2126729, 0.7151522, 0.0721750],
                              [0.0193339, 0.1191920, 0.9503041]])

for i, swatches in enumerate(SWATCHES):
    swatches_xyY = colour.XYZ_to_xyY(colour.RGB_to_XYZ(
        swatches, D65, D65, matrix_RGB_to_XYZ))
    ...
    swatches_f_xyY = colour.XYZ_to_xyY(colour.RGB_to_XYZ(
        swatches_f, D65, D65, matrix_RGB_to_XYZ))
    colour_checker = colour.characterisation.ColourChecker(
...
with open("D:\\meta_file.json", "r") as read_file:
...
loaded_model = pickle.load(open("D:\\GM_model.sav", 'rb'))
class_count = 5
...
for file in files:
    ...
    data_test[file] = []
    data_test[file].append(
        {"part": [{accept: float(int(wheat_part * 10000) / 10000)}, {reject: float(int(raps_part * 10000) / 10000)}],
         "area": [{accept: int(wheat_area)}, {reject: int(raps_area)}]
         })

data["output"] = {"analysis_results": {
    "accept": [{"accept": float(int((sum_wheat_part / (sum_wheat_part + sum_raps_part)) * 10000) / 10000)}],
    "reject": [{"reject": float(int((sum_raps_part / (sum_wheat_part + sum_raps_part)) * 10000) / 10000)}],
    "part_total": [{accept: float(int((sum_wheat_part / (sum_wheat_part + sum_raps_part)) * 10000) / 10000)},
                   {reject: float(int((sum_raps_part / (sum_wheat_part + sum_raps_part)) * 10000) / 10000)}],
